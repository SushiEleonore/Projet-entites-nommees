...
input_dir="processednew"
output_dir="eval/new5c"
pattern_file=pattern_enhanced1.txt
training_options=' -a sgd-l1 -t 3 -i 20 '
debug=0
verbose=0
...
echo "================ Training on $corpus_name (this may take some time) ================" 1>&2
...
echo "================ Inference on $corpus_name ================" 1>&2
...
echo "==== test on train ===="
echo "wapiti label -c -m $output_dir/$patname-train-$corpus_name.mod <(cat $input_dir/[A-S]*.tab) $output_dir/$patname-train-train-$corpus_name.tab" 1>&2
wapiti label -c -m $output_dir/$patname-train-$corpus_name.mod <(cat $input_dir/[A-S]*.tab) $output_dir/$patname-train-train-$corpus_name.tab
...
echo "==== total in $output_dir/$patname-train-all-$corpus_name.tab ===="
cat $output_dir/$patname-train-{train,test}-$corpus_name.tab > $output_dir/$patname-train-all-$corpus_name.tab

echo "================ Evaluation with conlleval.pl on $corpus_name ================" 1>&2
# evaluate the resulting entities
# $'\t' is a way to obtain a tabulation in bash
echo "$BINDIR/conlleval.pl -d $'\t' <$output_dir/$patname-train-test-$corpus_name.tab | tee $output_dir/$patname-train-test-$corpus_name.eval" 1>&2
$BINDIR/conlleval.pl -d $'\t' <$output_dir/$patname-train-test-$corpus_name.tab | tee $output_dir/$patname-train-test-$corpus_name.eval
